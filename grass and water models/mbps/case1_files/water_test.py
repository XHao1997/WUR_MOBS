...
tsim = np.linspace(0, 365, 366*24)
dt = 1/24

# Initial conditions
# TODO: define the dictionary of initial conditions
x0 = {'L1': 30,      #[mm] Water level in layer1
      'L2': 25,      #[mm] Water level in layer2
      'L3': 20,      #[mm] Water level in layer3
...
d = {'I_glb':np.array([tsim, np.full((tsim.size,), 400*100*100)]).T,
     'T':np.array([tsim, np.full((tsim.size,), 17)]).T,
     'f_prc':np.array([tsim, np.full((tsim.size,), 950/365)]).T,
