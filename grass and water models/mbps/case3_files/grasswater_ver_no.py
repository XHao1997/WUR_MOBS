...
t_weather = np.linspace(8, 364, 365)

# Grass data. (Organic matter assumed equal to DM) [gDM m-2]
# Groot and Lantinga (2004)
t_data = np.array([116, 131, 141, 151, 165,170,176,185,192,201,217])-1
m_data = np.array([39.1, 134.8, 260.9, 539.1, 695.6,100,126,203,258,348,587])
m_data = m_data / 1E3
# ---- Grass sub-model
# Step size
dt_grs = 1  # [d]
...
x0_grs = {'Ws': 1e-2, 'Wg': 2e-2}  # [kgC m-2]
...
T = pd.read_csv('../../data/norway_temp.csv')  # [0.1 °C] Env. temperature
a = pd.Series([0, 0, 0, 0, 0])  # Assuming this is your series 'a'

# Concatenate the existing 'Column1' with the Series 'a' and assign it back to 'Column1'
T = pd.concat((a, T['Air temperature']), ignore_index=True, axis=0).to_numpy()

I_gl = pd.read_csv('../../data/norway_rad.csv').to_numpy()*24  # [J cm-2 d-1] Global irr.

# T = T / 10  # [0.1 °C] to [°C] Environment temperature
I0 = 0.45 * I_gl * 1E4 / dt_grs  # [J cm-2 d-1] to [J m-2 d-1] Global irr. to PAR

d_grs = {'T': np.array([t_weather, T.flatten()]).T,
         'I0': np.array([t_weather, I0.flatten()]).T,
         }
...
a = pd.Series([0, 0, 0, 0, 0])  # Assuming this is your series 'a'
# Concatenate the existing 'Column1' with the Series 'a' and assign it back to 'Column1'
T = pd.concat((a, T['Air temperature']), ignore_index=True, axis=0).values

I_glb = pd.read_csv('../../data/norway_rad.csv').values*24  # [J cm-2 d-1] Global irr.
f_prc = pd.read_csv('../../data/norway_prec.csv')*24  # [0.1 mm d-1] Precipitation
a = pd.Series([0] * 5)  # Assuming this is your series 'a'
f_prc = pd.concat((a, f_prc['Precipitation (1 h)']), ignore_index=True, axis=0).values
...
f_prc = f_prc / dt_wtr  # [0.1 mm d-1] to [mm d-1] Precipitation

d_wtr = {'I_glb': np.array([t_weather, I_glb.flatten()]).T,
         'T': np.array([t_weather, T.flatten()]).T,
         'f_prc': np.array([t_weather, f_prc.flatten()]).T,
         }
...
for ti in it:
    ...
    if idx==165/5:
        c_Wg = grass.y['Wg'][idx*5]
        u_grs['f_Hr'] = c_Wg*0.2165
        # u_grs['f_Gr'] = c_Wg

        print(grass.y['Wg'][idx*5-1]/0.4)
    # Run grass model
