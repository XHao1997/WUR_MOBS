...
class Water(Module):
    ...
    * Mohtar, R.H., Buckmaster, D.R., & Fales, S.L. (1997),
    ...
    def diff(self, _t, _x0):
        ...
        if _f_prc > 0.2*S:
            Fru = (_f_prc - 0.2*S)**2 / (_f_prc + 0.8*S)
        else:
            Fru = 0
        Fpe = _f_prc - Fru
        ...
        delta = (5304/(_T+273)**2)*np.exp(21.3-5304/(_T+273))
        ET0 = (alpha * delta/(delta + gamma)) * Rn
        ETp = ET0*kcrop

        #Calculate the crop transpiration
        Ktr = 1 - np.exp(-0.6*_LAI)
        Tp = Ktr * ETp
        theta_pwp = np.array([theta_pwp1, theta_pwp2, theta_pwp3])
        theta_fc = np.array([theta_fc1, theta_fc2, theta_fc3])
        D = np.array([D1, D2, D3])
        fc = theta_fc * D
        pwp = theta_pwp * D
        Krf = np.array([krf1, krf2, krf3])

        WAIi = (_x0[0:3] - pwp) / (fc - pwp)
        WAIi[WAIi< 0] = 0
        Krt = np.array([])
        for i in range(len(WAIi)):
            if WAIi[i] < WAIc:
                Krt = np.append(Krt, (WAIi[i] / WAIc))
            else:
                Krt = np.append(Krt, 1)
        #print(Krt)
        Kra = 0.0408*np.exp(0.19*_T)
        Ftr = Krt * Kra * Krf * Tp
        print(Krt, Kra, Krf, Tp, _t)
        # Calculate the evaporation
        Ep = min(ETp - Tp, ETp*(1-mlc))
        Fev = Ep / np.sqrt(DSD)

        # Calculate water availability index
        #WAI = np.sum(np.array([_x0[0:3]]) - theta_pwp)/np.sum(theta_fc - theta_pwp)
        Fdr = np.zeros_like(_x0[0:3])
        # Calculate Drainage


        for i in range(len(_x0[0:3])):
            if _x0[i] > fc[i]:
                Fdr[i] = (_x0[i] - fc[i])/self.dt
            else:
                Fdr[i] = 0
        # print(Fdr)
        ...
        dL1_dt = Fpe - Ftr[0] - Fev - Fdr[0]
        dL2_dt = Fdr[0] - Ftr[1] - Fdr[1]
        dL3_dt = Fdr[1] - Ftr[2] - Fdr[2]
        dDSD_dt = 1 - (Fpe >= 1.5*Ep)*DSD
        
        # Store flows
        idx = np.isin(self.t, _t)
        self.f['f_Pe'][idx] = Fpe
        self.f['f_Tr1'][idx] = Ftr[0]
        self.f['f_Tr2'][idx] = Ftr[1]
        self.f['f_Tr3'][idx] = Ftr[2]
        self.f['f_Ev'][idx] = Fev
        self.f['f_Dr1'][idx] = Fdr[0]
        self.f['f_Dr2'][idx] = Fdr[1]
        self.f['f_Dr3'][idx] = Fdr[2]
